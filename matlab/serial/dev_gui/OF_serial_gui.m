...
fig = findobj('type', 'figure', 'tag', 'openfeeder');
...
uiSketchfactor = figSize(2)/105; % 175/105 mm => 1000x600 px
...
uiPopDay = uicontrol(fig, ...
    'style', 'popupmenu', ...
    'units', 'pixels', ...
    'position', [26 84 7 5]*uiSketchfactor, ...
    'tag', 'TimDay', ...
...
uiPopMonth = uicontrol(fig, ...
    'style', 'popupmenu', ...
    'units', 'pixels', ...
    'position', [34 84 7 5]*uiSketchfactor, ...
    'tag', 'TimeMonth', ...
...
uiPopYear = uicontrol(fig, ...
    'style', 'popupmenu', ...
    'units', 'pixels', ...
    'position', [42 84 7 5]*uiSketchfactor, ...
    'tag', 'TimeYear', ...
    'string',  cellstr(num2str((1:30).', '%02d')), ...
    'fontweight', 'bold', ...
    'value', str2double(datestr(now, 'yy')));
uiPopHour = uicontrol(fig, ...
    'style', 'popupmenu', ...
    'units', 'pixels', ...
    'position', [26 79.5 7 5]*uiSketchfactor, ...
    'tag', 'TimeHour', ...
...
uiPopMinute = uicontrol(fig, ...
    'style', 'popupmenu', ...
    'units', 'pixels', ...
    'position', [34 79.5 7 5]*uiSketchfactor, ...
    'tag', 'TimeMinute', ...
...
uiPopSecond = uicontrol(fig, ...
    'style', 'popupmenu', ...
    'units', 'pixels', ...
    'position', [42 79.5 7 5]*uiSketchfactor, ...
    'tag', 'TimeSecond', ...
    'string',  cellstr(num2str((0:59).', '%02d')), ...
    'fontweight', 'bold', ...
...
uicontrol('style', 'frame', ...
    'units', 'pixels', ...
    'position', [52.5 3 frame_width 100]*uiSketchfactor, ...
    'backgroundcolor', frame_color);
...
uiButtonListCommand = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [55 98 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Commands', ...
...
uiButtonEmptyBuffer = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [70 98 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Clear UART', ...
...
uiButtonQuit = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [85 98 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Quit UART', ...
    'tag', 'uiButtonquit', ...
    'enable', 'off', ...
...
uicontrol('style', 'frame', ...
    'units', 'pixels', ...
    'position', [55 96 45 frame_width]*uiSketchfactor, ...
    'backgroundcolor', frame_color);

uiButtonInitLEDs = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [55 90 button_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Init LEDs', ...
    'tag', 'uiButtonInitLEDs', ...
    'enable', 'off', ...
    'callback', {@sendCommand, 'ai'});

uiButtonTestLEDs = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [80 90 button_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Test LEDs fast', ...
    'tag', 'uiButtonTestLEDs', ...
    'enable', 'off', ...
    'callback', {@sendCommand, 'at'});

uiButtonTestLEDsOrder = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [55 85 button_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Test LEDs order', ...
    'tag', 'uiButtonTestLEDsOrder', ...
    'enable', 'off', ...
    'callback', {@sendCommand, 'ao'});

uiButtonTestLEDsColor = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [80 85 button_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Test LEDs color', ...
    'tag', 'uiButtonTestLEDsColor', ...
    'enable', 'off', ...
    'callback', {@sendCommand, 'ac'});

uicontrol(fig, ...
    'style', 'text', ...
    'units', 'pixels', ...
    'position', [56 79 5 5]*uiSketchfactor, ...
    'string', 'R', ...
    'horizontalalignment', 'left', ...
    'fontweight', 'bold');

uiEditLedsRed = uicontrol(fig, ...
    'style', 'edit', ...
    'units', 'pixels', ...
    'position', [58 80 edit_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', '0', ...
    'tag', 'uiEditLedsRed', ...
    'enable', 'off');
...
uicontrol(fig, ...
    'style', 'text', ...
    'units', 'pixels', ...
    'position', [64 79 5 5]*uiSketchfactor, ...
    'string', 'G', ...
    'horizontalalignment', 'left', ...
    'fontweight', 'bold');

uiEditLedsGreen = uicontrol(fig, ...
    'style', 'edit', ...
    'units', 'pixels', ...
    'position', [66 80 edit_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', '0', ...
    'tag', 'uiEditLedsGreen', ...
    'enable', 'off');
...
uicontrol(fig, ...
    'style', 'text', ...
    'units', 'pixels', ...
    'position', [72 79 5 5]*uiSketchfactor, ...
    'string', 'B', ...
    'horizontalalignment', 'left', ...
    'fontweight', 'bold');

uiEditLedsBlue = uicontrol(fig, ...
    'style', 'edit', ...
    'units', 'pixels', ...
    'position', [74 80 edit_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', '0', ...
    'tag', 'uiEditLedsBlue', ...
    'enable', 'off');

uiButtonLEDsRGB = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [80 80 button_size]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'set RGB', ...
    'tag', 'uiButtonLEDsRGB', ...
    'enable', 'off', ...
...
uicontrol('style', 'frame', ...
    'units', 'pixels', ...
    'position', [55 79 45 frame_width]*uiSketchfactor, ...
    'backgroundcolor', frame_color);

uiButtonReopenSecurity = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [55 73 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Reopen secu.', ...
    'tag', 'uiButtonReopenSecurity', ...
    'enable', 'off', ...
    'callback', {@sendCommand, 'xr'});

uiButtonGuillotineSecurity = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [70 73 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Guillot. secu.', ...
    'tag', 'uiButtonGuillotineSecurity', ...
    'enable', 'off', ...
    'callback', {@sendCommand, 'xg'});

uiButtonGuillotineOffsetSecurity = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [85 73 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Guillot. offset', ...
    'tag', 'uiButtonGuillotineOffsetSecurity', ...
    'enable', 'off', ...
    'callback', @setGuillotineOffset);

uiButtonMaxOffsetSecurity = uicontrol(fig, ...
    'units', 'pixels', ...
    'position', [55 67 button_size_2]*uiSketchfactor, ...
    'fontweight', 'bold', ...
    'string', 'Max. offset', ...
    'tag', 'uiButtonMaxOffsetSecurity', ...
    'enable', 'off', ...
    'callback', @setMaxPosOffset);

%% Preview zone
uiCommunicationWindow = uicontrol(fig, ...
    'style', 'listbox', ...
    'units', 'pixels', ...
    'position', [103.5 0 70 105]*uiSketchfactor, ...
    'horizontalalignment', 'left', ...
    'fontweight', 'bold', ...
    'fontname', 'monospaced', ...
    'fontsize', 10, ...
...
uiUartRxFrame = uicontrol('style', 'frame', ...
    'units', 'pixels', ...
    'position', [173.5 0 2 106]*uiSketchfactor, ...
    'tag', 'uiUartRxFrame', ...
    'backgroundcolor', frame_color);
...
timerReadData = timer('ExecutionMode', 'fixedDelay', 'Period', 0.01, 'TimerFcn', @readDataFromOF, 'tag', 'OF_serial_timer');
    ...
    function connectCOM(~, ~)
        ...
        set(uiButtonQuit, 'enable', 'on')
        
        set(uiButtonInitLEDs, 'enable', 'on')
        set(uiButtonTestLEDs, 'enable', 'on')
        set(uiButtonTestLEDsOrder, 'enable', 'on')
        set(uiButtonTestLEDsColor, 'enable', 'on')
        
        set(uiEditLedsRed, 'enable', 'on')
        set(uiEditLedsGreen, 'enable', 'on')
        set(uiEditLedsBlue, 'enable', 'on')
        
        set(uiButtonLEDsRGB, 'enable', 'on')
   
        set(uiButtonReopenSecurity, 'enable', 'on')
        set(uiButtonGuillotineSecurity, 'enable', 'on')
        set(uiButtonGuillotineOffsetSecurity, 'enable', 'on')
        set(uiButtonMaxOffsetSecurity, 'enable', 'on')
    ...
    function disconnectCOM(~, ~)
        ...
        set(uiButtonQuit, 'enable', 'off')
                
        set(uiButtonInitLEDs, 'enable', 'off')
        set(uiButtonTestLEDs, 'enable', 'off')
        set(uiButtonTestLEDsOrder, 'enable', 'off')
        set(uiButtonTestLEDsColor, 'enable', 'off')
        
        set(uiEditLedsRed, 'enable', 'off')
        set(uiEditLedsGreen, 'enable', 'off')
        set(uiEditLedsBlue, 'enable', 'off')
        
        set(uiButtonLEDsRGB, 'enable', 'off')
        
        set(uiButtonReopenSecurity, 'enable', 'off')
        set(uiButtonGuillotineSecurity, 'enable', 'off')
        set(uiButtonGuillotineOffsetSecurity, 'enable', 'off')
        set(uiButtonMaxOffsetSecurity, 'enable', 'off')
...
%     end
    ...
    function setLedsRgb(~, ~)        
        ...
        r = get(uiEditLedsRed, 'string');
        g = get(uiEditLedsGreen, 'string');
        b = get(uiEditLedsBlue, 'string');
        
        r = str2double(r);
        g = str2double(g);
        b = str2double(b);
        
        if any(isnan([r g b])) || r < 0 || g < 0 || b < 0 || r > 255 || g > 255 || b > 255
            errordlg('Color must be a numerical value between 0 and 255')
        end
        
        fwrite(serialObj, uint8(['ar' num2str(r, '%04d')]));
        fwrite(serialObj, uint8(['ag' num2str(g, '%04d')]));
        fwrite(serialObj, uint8(['ab' num2str(b, '%04d')]));        
        
    end

    function setGuillotineOffset(~, ~) 
        
        offset = inputdlg('Enter guillotime time offset in milliseconds', 'Openfeeder', ...
            1, {'5000'});
        ...
        if offset{1} < 0 
            errordlg('Time offset must be positive')
            return            
        end
        
        offset = str2double(offset{1});
        
        fwrite(serialObj, uint8(['xo' num2str(offset, '%04d')]));
        
    end
     
    function setMaxPosOffset(~, ~) 
        
        offset = inputdlg('Enter max position offset', 'Openfeeder', ...
            1, {'150'});
        ...
        if offset{1} < 0 
            errordlg('Position offset must be positive')
            return            
        end
        
        offset = str2double(offset{1});
        
        fwrite(serialObj, uint8(['xp' num2str(offset, '%04d')]));
